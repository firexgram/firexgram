...
object TranslateAPI {
    ...
    fun callTranslationDialog(msg: String, act: AppCompatActivity) {
        if (CatogramConfig.oldTranslateUI)
            ...
            TranslateAlert.showAlertCato(act, null, LocaleController.getString("LanguageCode", R.string.LanguageCode), msg, false)
        }
    }

    @JvmStatic
    fun callTranslationDialog(msgobj: MessageObject, act: AppCompatActivity, chatActivity: BaseFragment, b: Boolean) {
        val msg = extractMessageText(msgobj)
        if (CatogramConfig.oldTranslateUI)
            TranslationSheetFragment(msg, b).show(act.supportFragmentManager, null)
        else {
            TranslateAlert.showAlertCato(act, chatActivity, LocaleController.getString("LanguageCode", R.string.LanguageCode), msg, b)
        }
    }
